...
export function publicStatusPageUrl(
  ...
  const absolute = publicServiceUrl("status", baseUrl);
  return absolute ? `${absolute}#${encodeURIComponent(id)}` : allowRelative ? relative : null;
}

export function publicServiceUrl(path, baseUrl = process.env.FIXLOOP_PUBLIC_BASE_URL) {
  ...
  try {
    ...
    parsed.search = "";
    parsed.hash = "";
    return `${parsed.href.replace(/\/$/, "")}/${String(path).replace(/^\/+/, "")}`;
  } catch {
    return null;
  }
}
